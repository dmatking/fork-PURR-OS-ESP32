...
static void shell_paint(mw_handle_t handle, const mw_gl_draw_info_t *d)
{
    ...
    // Start button — sunken when menu is open, raised otherwise
    if (smenu_open)
    ...
    if (about_open) {
        ...
        mw_gl_string(d, ABOUT_X + 8, ABOUT_Y + 42, "ESP32-2432S024C  (CYD)");
...
void mw_user_root_paint_function(const mw_gl_draw_info_t *draw_info)
{
    ...
    mw_gl_set_border(MW_GL_BORDER_OFF);
    mw_gl_set_solid_fill_colour(WCE_DESKTOP);
    mw_gl_rectangle(draw_info, 0, 0, SCR_W, SCR_H);
}
