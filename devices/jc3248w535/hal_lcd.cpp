...
int16_t mw_hal_lcd_get_display_width(void)  { return ST7796_TFT_WIDTH;  }
int16_t mw_hal_lcd_get_display_height(void) { return ST7796_TFT_HEIGHT; }

void mw_hal_lcd_pixel(int16_t x, int16_t y, mw_hal_lcd_colour_t colour) {
    display_st7796_push_block(x, y, 1, 1, to_rgb565(colour));
}

void mw_hal_lcd_filled_rectangle(int16_t start_x, int16_t start_y,
                                  int16_t width, int16_t height,
                                  mw_hal_lcd_colour_t colour) {
    display_st7796_push_block(start_x, start_y, width, height, to_rgb565(colour));
}
...
void mw_hal_lcd_monochrome_bitmap_clip(int16_t image_start_x, int16_t image_start_y,
    ...
    for (int16_t row = 0; row < (int16_t)bitmap_height; row++) {
        ...
        display_st7796_push_colors(image_start_x + x0, sy, x1 - x0, 1, row_buf);
    }
}
