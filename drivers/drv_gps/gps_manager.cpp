...
void gps_manager_init() {
    ...
    if (probe_baud(GPS_UART_BAUD_DEFAULT, 800)) { s_enabled = true; return; }
    if (probe_baud(GPS_UART_BAUD_FALLBACK, 800)) { s_enabled = true; return; }
    ESP_LOGW(TAG, "GPS module not detected");
}

void gps_manager_tick() {
    if (!s_enabled) return;
...
static int gps_cmd(const char *args, char *out, int out_len) {
    if (!args || args[0] == '\0' || strcmp(args, "status") == 0) {
        snprintf(out, out_len, "gps %s fix=%s lat=%.6f lon=%.6f",
                 gps_manager_enabled() ? "enabled" : "offline",
                 gps_manager_has_fix() ? "yes" : "no",
                 gps_manager_latitude(), gps_manager_longitude());
        return 0;
    }
    snprintf(out, out_len, "usage: gps [status]");
    return -1;
}

static sys_drv_t s_gps_drv = {
    .name      = "gps",
    .subsystem = "io",
    .enabled   = false,
    .init      = gps_manager_init,
    .tick      = gps_manager_tick,
    .deinit    = gps_manager_deinit,
    .cmd       = gps_cmd,
};

void gps_manager_drv_register(bool enabled) {
    s_gps_drv.enabled = enabled;
    sys_drv_register(&s_gps_drv);
}
