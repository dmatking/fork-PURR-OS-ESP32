...
void wifi_manager_tick() {}  // events handled asynchronously via esp_event
...
static int wifi_cmd(const char *args, char *out, int out_len) {
    if (!args || args[0] == '\0' || strcmp(args, "status") == 0) {
        snprintf(out, out_len, "wifi %s rssi=%d",
                 wifi_manager_connected() ? "connected" : "disconnected",
                 wifi_manager_connected() ? wifi_manager_rssi() : 0);
        return 0;
    }
    if (strcmp(args, "on") == 0)  { wifi_manager_enable();     snprintf(out, out_len, "ok"); return 0; }
    if (strcmp(args, "off") == 0) { wifi_manager_disable();    snprintf(out, out_len, "ok"); return 0; }
    if (strcmp(args, "scan") == 0){ wifi_manager_scan_start(); snprintf(out, out_len, "scan started"); return 0; }
    snprintf(out, out_len, "usage: wifi [status|on|off|scan]");
    return -1;
}

static sys_drv_t s_wifi_drv = {
    .name      = "wifi",
    .subsystem = "net",
    .enabled   = false,
    .init      = wifi_manager_init,
    .tick      = wifi_manager_tick,
    .deinit    = wifi_manager_deinit,
    .cmd       = wifi_cmd,
};

void wifi_manager_drv_register(bool enabled) {
    s_wifi_drv.enabled = enabled;
    sys_drv_register(&s_wifi_drv);
}
