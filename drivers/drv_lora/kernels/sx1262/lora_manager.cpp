// lora_manager.cpp — SX1262 via RadioLib (pure IDF, no Arduino)
// Legacy kernel slot; main drv_lora/lora_manager.cpp is the preferred sx1262 impl.
// This file is selected when PURR_LORA_KERNEL=sx1262 AND a kernels/sx1262/ override is wanted.
...
void lora_manager_init(uint32_t freq_hz, uint8_t power_dbm) {
    ...
    lora_ready = true;
    ESP_LOGI(TAG, "SX1262 OK %.1f MHz %d dBm", freq_mhz, power_dbm);
}
