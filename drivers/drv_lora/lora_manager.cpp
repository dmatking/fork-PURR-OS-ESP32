...
static uint32_t s_reg_freq  = 915000000;
static uint8_t  s_reg_power = 14;

static void lora_init_wrap() { lora_manager_init(s_reg_freq, s_reg_power); }

static int lora_cmd(const char *args, char *out, int out_len) {
    if (!args || args[0] == '\0' || strcmp(args, "status") == 0) {
        snprintf(out, out_len, "lora %s rssi=%d snr=%.1f freq=%luHz",
                 lora_manager_enabled() ? "ready" : "offline",
                 lora_manager_rssi(), (double)lora_manager_snr(),
                 (unsigned long)lora_manager_get_frequency());
        return 0;
    }
    if (strcmp(args, "rx?") == 0) {
        snprintf(out, out_len, lora_manager_data_available() ? "data ready" : "no data");
        return 0;
    }
    snprintf(out, out_len, "usage: lora [status|rx?]");
    return -1;
}

static sys_drv_t s_lora_drv = {
    .name      = "lora",
    .subsystem = "radio",
    .enabled   = false,
    .init      = lora_init_wrap,
    .tick      = lora_manager_tick,
    .deinit    = lora_manager_deinit,
    .cmd       = lora_cmd,
};

void lora_manager_drv_register(bool enabled, uint32_t freq_hz, uint8_t power_dbm) {
    s_reg_freq  = freq_hz;
    s_reg_power = power_dbm;
    s_lora_drv.enabled = enabled;
    sys_drv_register(&s_lora_drv);
}
