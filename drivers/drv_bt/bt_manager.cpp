...
static int bt_cmd(const char *args, char *out, int out_len) {
    if (!args || args[0] == '\0' || strcmp(args, "status") == 0) {
        snprintf(out, out_len, "bt %s paired=%d",
                 bt_manager_enabled() ? "enabled" : "disabled",
                 bt_manager_paired_count());
        return 0;
    }
    if (strcmp(args, "on") == 0)   { bt_manager_enable();  snprintf(out, out_len, "ok"); return 0; }
    if (strcmp(args, "off") == 0)  { bt_manager_disable(); snprintf(out, out_len, "ok"); return 0; }
    if (strcmp(args, "scan") == 0) { bt_manager_start_discovery(8000); snprintf(out, out_len, "scanning 8s"); return 0; }
    snprintf(out, out_len, "usage: bt [status|on|off|scan]");
    return -1;
}

static sys_drv_t s_bt_drv = {
    .name      = "bt",
    .subsystem = "net",
    .enabled   = false,
    .init      = bt_manager_init,
    .tick      = bt_manager_tick,
    .deinit    = bt_manager_deinit,
    .cmd       = bt_cmd,
};

void bt_manager_drv_register(bool enabled) {
    s_bt_drv.enabled = enabled;
    sys_drv_register(&s_bt_drv);
}
