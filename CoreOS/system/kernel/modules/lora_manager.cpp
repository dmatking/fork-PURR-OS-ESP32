...
void lora_manager_init(uint32_t freq_hz, uint8_t power_dbm) {
    ...
    lora_ready = true;
    Serial.printf("[lora] SX1262 OK %lu Hz %d dBm\n", freq_hz, power_dbm);
}
...
void lora_manager_set_frequency(uint32_t freq_hz) {
    lora_freq = freq_hz;
    if (lora_ready) LoRa.setFrequency(freq_hz);
}
...
float lora_manager_snr()  { return lora_ready ? LoRa.packetSnr()  : 0; }
bool  lora_manager_busy() { return false; }  // SX1262 async TX not exposed by Heltec lib
...
bool lora_manager_data_available() {
    return (!lora_yield_flag && lora_ready && LoRa.parsePacket() > 0);
}
...
void lora_manager_yield()   { lora_yield_flag = true;  if (lora_ready) LoRa.sleep(); }
void lora_manager_reclaim() { lora_yield_flag = false; if (lora_ready) LoRa.idle();  }
