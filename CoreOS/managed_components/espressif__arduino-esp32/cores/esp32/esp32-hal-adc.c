...
arduino_adc_cont_data_t *adc_result = NULL;
...
bool analogContinuous(const uint8_t pins[], size_t pins_count, uint32_t conversions_per_pin, uint32_t sampling_freq_hz, void (*userFunc)(void)) {
  ...
  adc_result = malloc(pins_count * sizeof(arduino_adc_cont_data_t));
...
bool analogContinuousRead(arduino_adc_cont_data_t **buffer, uint32_t timeout_ms) {
