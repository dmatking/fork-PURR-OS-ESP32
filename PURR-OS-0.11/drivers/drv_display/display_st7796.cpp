...
void display_st7796_init() {
    ...
    // Clear GRAM before enabling display to avoid white flash on boot
    display_st7796_fill_rect(0, 0, ST7796_TFT_WIDTH, ST7796_TFT_HEIGHT, 0x0000);
...
void display_st7796_push_block(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color) {
    display_st7796_fill_rect(x, y, w, h, color);
}
...
static sys_drv_t s_st7796_drv = {
    .name="display:st7796",.subsystem="display",.enabled=false,
    .init=display_st7796_init,.tick=display_st7796_tick,.deinit=display_st7796_deinit,.cmd=NULL
};
void display_st7796_drv_register(bool enabled){s_st7796_drv.enabled=enabled;sys_drv_register(&s_st7796_drv);}
