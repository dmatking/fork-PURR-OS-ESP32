...
bool device_config_default(device_config_t* out) {
    ...
    strlcpy(out->display, "axs15231b",  sizeof(out->display));
    strlcpy(out->touch,   "axs15231b", sizeof(out->touch));
    out->display_w = 320; out->display_h = 480;
    out->psram = true; out->flash_mb = 16; out->sd = false; // SD pins unverified — enable once confirmed
    out->wifi = true; out->bt = true;
